...
import info.meysam.veoapp.data.local.LaunchDetailRoomDatabase
...
class VeoApplication : Application() {
    ...
    val database by lazy { LaunchDetailRoomDatabase.getDatabase(this) }
    val service by lazy { LaunchService.instance }
    val repository by lazy { LaunchRepository(service,database.launchDetailDao()) }
}